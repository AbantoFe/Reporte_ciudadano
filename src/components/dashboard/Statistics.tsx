  ...
  const loadStatistics = async () => {
    try {
      const { data: incidents, error } = await supabase
        ...
        .select(`
          *,
          incident_categories(name, color)
        `);
      ...
      const inProgress = incidents?.filter((i) => i.status === 'in_progress').length || 0;
      const resolved = incidents?.filter((i) => i.status === 'resolved').length || 0;
      const rejected = incidents?.filter((i) => i.status === 'rejected').length || 0;

      const categoryMap = new Map<string, { count: number; color: string }>();
      incidents?.forEach((incident) => {
        const categoryName = incident.incident_categories?.name || 'Sin categoría';
        const categoryColor = incident.incident_categories?.color || '#6B7280';
